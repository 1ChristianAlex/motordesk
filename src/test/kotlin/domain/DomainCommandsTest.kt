...
class CreateNewCompanyUseCaseCommandTest {
    @Test fun `retains company input`() {
...
class CreateNewCompanyUseCaseErrorTest {
    @Test fun `identifies duplicate company`() {
...
class SearchCompanyByCnpjUseCaseCommandTest {
    @Test fun `retains search CNPJ`() = assertEquals("12345678000195", SearchCompanyByCnpjUseCaseCommand(CNPJ("12345678000195")).cnpj.value)
}

class DecrementItemInventoryCommandTest {
    @Test fun `retains item and quantity`() = assertEquals(2, DecrementItemInventoryCommand(1, 2).quantity)
}

class CreateServiceOrderCommandTest {
    @Test fun `retains service order references`() {
...
class DeleteServiceOrderCommandTest {
    @Test fun `retains cancellation reason`() = assertEquals("Cancelled", DeleteServiceOrderCommand("Cancelled", 1).complaint)
}

class GetClientServiceOrdersByCodeCommandTest {
    @Test fun `retains ownership constraint`() = assertEquals(2, GetClientServiceOrdersByCodeCommand("#code", 2).userId)
}
...
class UpdateServiceOrderTaskCommandTest {
    @Test fun `retains progress update`() {
...
class CreateNewUserUseCaseCommandTest {
    @Test fun `retains aggregate input`() {
...
class VerifyIsUserDataAvailableUseCaseCommandTest {
    @Test fun `retains unique user data`() {
...
class UserNotFoundExceptionTest {
    @Test fun `contains missing user id`() = assertEquals("No user found with id 7", UserNotFoundException(7).message)
}

class InvalidCredentialsExceptionTest {
    @Test fun `does not disclose credential details`() = assertEquals("User not found with provided credentials", InvalidCredentialsException().message)
}
