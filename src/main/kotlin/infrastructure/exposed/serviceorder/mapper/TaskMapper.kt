...
suspend fun TaskEntity.toModel(serviceOrderId: Int): Task = suspendTransaction {
    Task(
        ...
        isActive = isActive,
        category = category,
        status = ServiceOrderTasksTable.select(ServiceOrderTasksTable.status)
            .where { ServiceOrderTasksTable.serviceOrder eq serviceOrderId }.map { it[ServiceOrderTasksTable.status] }
            .first()
    )
}
