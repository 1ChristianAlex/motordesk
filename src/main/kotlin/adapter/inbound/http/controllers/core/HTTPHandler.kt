...
import com.khrix.adapter.inbound.http.controllers.core.exceptions.HandlerException.Companion.fromValidationErrorResult
import com.khrix.adapter.inbound.http.controllers.core.exceptions.HandlerException.Companion.toHttpResultError
...
abstract class BaseHTTPHandler<Body, Output> : HTTPHandler<Body, Output> {
    ...
    override suspend fun handler(body: Body): HttpResult<Output> =
        try {
            ...
            fromValidationErrorResult(error)
        } catch (exception: Exception) {
            logger.error(exception.message, exception.cause)
            toHttpResultError(HandlerException.BadRequest(exception))
        }
