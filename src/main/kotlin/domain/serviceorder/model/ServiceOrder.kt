...
data class ServiceOrder(
    ...
    private fun validation() =
        Validation.Companion<ServiceOrder> {
            ServiceOrder::client {
    ...
    private fun calculateTotal(): BigDecimal =
        BigDecimal.ZERO
            .add(
                tasks.fold(BigDecimal.ZERO) { acc, task ->
                    acc.add(task.price.value)
                },
            ).add(
                inventoryItems.fold(BigDecimal.ZERO) { acc, item ->
    ...
    fun codeIds(): List<Int> = listOf(id, client.id, operator.id, vehicle.id) + tasks.map { it.id } + inventoryItems.map { it.id }
}
