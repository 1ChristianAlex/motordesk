...
data class Address(
    ...
    private fun validation(): Validation<Address> {
        val pattern = "^[a-zA-ZáéíóúãõçÁÉÍÓÚÃÕÇ\\s.'-]+$"
        return Validation {
            Address::street {
                minLength(3) hint "Street must be at least 3 characters long"
                maxLength(100) hint "Street must not exceed 100 characters"
                pattern(pattern) hint "Street can only contain letters, spaces, dots, hyphens, and apostrophes"
            }
            Address::number {
