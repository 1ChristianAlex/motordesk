# GitHub AI configuration

This directory contains repository instructions for AI-assisted development and code review.

- `copilot-instructions.md` — repository-wide GitHub Copilot review rules.
- `instructions/domain.instructions.md` — domain-layer rules.
- `instructions/infrastructure.instructions.md` — adapter and external integration rules.
- `instructions/api.instructions.md` — HTTP/API review rules.
- `instructions/tests.instructions.md` — testing review rules.

The repository-level `AGENTS.md` remains the broader source of project context for AI agents. These files complement it
with GitHub Copilot-specific review guidance.
